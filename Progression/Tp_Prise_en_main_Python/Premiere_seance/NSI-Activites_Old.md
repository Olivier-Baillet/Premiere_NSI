...
Pour écrire nos programmes en Python, nous utiliserons le logiciel Geany.
Une fois Geany lancé, vous devriez obtenir quelque chose qui ressemble à cela :

![image.png](NSI%20Activit%C3%A9s%20Old/image.png)

Afin de bien démarrer, commencer par cliquer sur l’onglet :

> Fichier → Nouveau (selon un modèle) → main.py
> 

![image.png](NSI%20Activit%C3%A9s%20Old/image%201.png)

Cela vient de créer un document de départ.  Enregistrez le fichier dans un dossier qui vous servira de dossier de travail, avec le nom :

> Programme.py
> 

Geany se divise en plusieurs fenêtres, mais la fenêtre qui va principalement nous intéresser, est la fenêtre centrale dont l’onglet doit être intitulé maintenant “Programme.py”

Maintenant dans cette fenêtre, saisissez le programme suivant à la place du “return 0”:
...
![image.png](NSI%20Activit%C3%A9s%20Old/image%202.png)

Cliquez sur les "trois écroux" en dessous de l’onglet “Outils” et “Aide”, afin d'exécuter le programme qui vient d'être saisi.

![image.png](NSI%20Activit%C3%A9s%20Old/image%203.png)

Vous devez voir le message "hello world !" apparaître dans la console qui vient d’apparaître.

![image.png](NSI%20Activit%C3%A9s%20Old/image%204.png)

## **Affectation**

A la suite de notre “hello world”, dans la fenêtre centrale de Geany, saisissez le code suivant :

```python
point_de_vie = 15
```

Executer le code, comme précédement.

Vous constaterez que rien n’a changé dans la console qui c’est affiché.

<aside>
💡

Le symbole `=` ici utilisé n'a **rien à voir** avec le symbole = utilisé en mathématique. On dit qu'on a **affecté** à `point_de_vie` la valeur 32, et il faut se représenter mentalement cette action par l'écriture `point_de_vie ← 32`.
...
• Dans un premier temps vous saisisez votre programme
...
![image.png](NSI%20Activit%C3%A9s%20Old/image%205.png)

L’accolade de la fonction main(args) doit être continue, si ce n’est pas le cas comme ci-dessous, vous risquez d’avoir des erreurs :

![image.png](NSI%20Activit%C3%A9s%20Old/image%206.png)
...
Que constatez-vous dans la console lorsque vous executer le nouveau programme ?

Vous devriez obtenir une erreur dans la console, la dernière ligne est la plus importante ici :

![image.png](NSI%20Activit%C3%A9s%20Old/image%207.png)
...
Creer une variable avec *“votre nom”* pour qui on affecte la valeur *“votre âge”*.

Tester le programme pour voir s’il n’y a pas d’erreur à l’execution.
